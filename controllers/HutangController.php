<?php
class HutangController extends FController
{
	...
	public function actionIndex()
	{
		...
		if ($allow_read) {
			...
			$hutangs = Hutang::model()->with('Supplier')->findAll($Criteria);
	...
	public function actionViewHutang()
	{
	  	$menuid = 30;
		$userid_actor = Yii::app()->request->getParam('userid_actor');
	  	...
	  	if ($allow_read) {
			...
			$bread_crumb_list =
				'<li>Keuangan</li>'.
				'<li>></li>'.
				'<li><a href="#" onclick="ShowList('.$userid_actor.');">Hutang</a></li>'.
				'<li>></li>'.
				'<li>View Hutang</li>';
		...
		if(isset($do_add))
		{
			if($do_add == 1)
			{
				...
				if($form->validate())
				{
					...
					$hutang['is_paid'] = $form['is_paid'];
					$hutang['payment_date'] = $form['payment_date'];
					$hutang['payment_code'] = $form['payment_code'];
					$hutang['status_input'] = $form['status_input'];
					$hutang['status_payment'] = $form['status_payment'];
					$hutang['date_created'] = new CDbExpression('NOW()');
					$hutang['created_by'] = FHelper::GetUserName($userid_actor);
					...
					$data = "$hutang[hutang_id], $hutang[tgl], $hutang[faktur_no], $hutang[po_no], $hutang[supplier_id], $hutang[total], ".
							"$hutang[note], $hutang[is_paid], $hutang[payment_date], $hutang[payment_code], $hutang[status_input], ".
							"$hutang[status_payment], $hutang[date_created], $hutang[created_by], $hutang[date_update], $hutang[update_by], 1";

					FAudit::add('KEUANGANHUTANG', 'Add', FHelper::GetUserName($userid_actor), $data);
					...
					'<p><strong>Success notification:</strong> Data faktur '.$hutang['faktur_no'].' tgl '.date("d-m-Y", strtotime($hutang['tgl'])).' berhasil ditambah</p>'.
					'</div>';
			...
			else
			{
				...
				$hutangs = Hutang::model()->findAll($Criteria);
	...
	public function actionEditHutang()
	{
	  	$menuid = 30;
	...
	*/
	public function actionSelectionHutang()
	{
		$menuid = 30;
		$userid_actor = Yii::app()->request->getParam('userid_actor');
